-- Fix: Add created_at to get_recommendations return
-- Date: January 22, 2026
-- Issue: Recency badges not showing because created_at was missing from return
...
RETURNS TABLE (
  ...
  created_at TIMESTAMPTZ  -- Added: needed for recency badge
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    WHERE p.is_available = true
      ...
      AND NOT EXISTS (
        ...
        AND (
          ...
          OR
          -- 2+ impressions: hide for 1 month (unless 50+ likes)
  ...
  SELECT 
    ...
    bl.total_score,
    bl.reason,
    bl.created_at  -- Added: return created_at for recency badge
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed get_recommendations to include created_at in return.';
  RAISE NOTICE 'Recency badges should now display correctly.';
END $$;
