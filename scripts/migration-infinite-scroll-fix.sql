-- Migration: Fix infinite scroll by not excluding shown products
-- Date: December 29, 2025
-- Purpose: Allow infinite scroll by including all products but deprioritizing recently shown ones
...
BEGIN
  ...
  -- Count ALL available products (not liked by user) for infinite scroll
  -- Don't exclude based on impressions - we want true infinite scroll
  ...
  -- Infinite scroll: cycle back to start when offset exceeds available
  ...
  scored_products AS (
    SELECT 
      ...
      -- Check if recently shown (for scoring penalty, not exclusion)
      ...
      ((EXTRACT(DOY FROM NOW())::int + refresh_seed + EXTRACT(EPOCH FROM p.created_at)::bigint % 1000) % 8) as variety_score,
      
      -- Discovery bonus: adds randomization based on seed
      (abs(('x' || substr(p.id::text, 1, 8))::bit(32)::int + refresh_seed) % 12) as discovery_bonus,
      
      -- PENALTY for recently shown products (deprioritize but don't exclude)
      -- Products shown recently get pushed down in the ranking
      CASE 
        WHEN ri.product_id IS NOT NULL THEN -15  -- Penalty for recently shown
        ELSE 0
    ...
    WHERE p.is_available = true
      -- Only exclude already-liked products, NOT recently shown ones
  ),
  ranked_products AS (
    SELECT 
      sp.*,
      (sp.brand_score + sp.category_score + sp.freshness_score + sp.popularity_score + 
       sp.price_score + sp.sale_bonus + sp.variety_score + sp.discovery_bonus + sp.recency_penalty) as total_score,
      CASE 
      ...
      ROW_NUMBER() OVER (PARTITION BY sp.brand_id ORDER BY 
        (sp.brand_score + sp.category_score + sp.freshness_score + sp.popularity_score + 
         sp.price_score + sp.sale_bonus + sp.variety_score + sp.discovery_bonus + sp.recency_penalty) DESC,
        sp.created_at DESC
      ) as brand_rank
...
DO $$
BEGIN
  RAISE NOTICE '=== Infinite Scroll Fix Migration Complete ===';
  RAISE NOTICE '';
  RAISE NOTICE 'Changes:';
  RAISE NOTICE '  - Products are no longer excluded based on recent impressions';
  RAISE NOTICE '  - Recently shown products get a -15 score penalty instead';
  RAISE NOTICE '  - Infinite scroll now cycles through ALL available products';
  RAISE NOTICE '  - Already-liked products are still excluded';
END $$;
