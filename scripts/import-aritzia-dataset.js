#!/usr/bin/env node
/**
 * Import Aritzia products from Apify dataset
 * 
 * Quick script to import products from an existing Apify dataset
 * without running the actor again. Same workflow as Free People.
 */
...
if (!datasetId) {
  ...
  console.log('Usage: node scripts/import-aritzia-dataset.js <dataset_id>');
  process.exit(1);
}
...
async function importDataset() {
  console.log('🏷️  Importing Aritzia products from Apify dataset...');
  console.log(`📦 Dataset ID: ${datasetId}\n`);

  try {
    // Get Aritzia brand from database
    ...
    if (brandError || !brand) {
      throw new Error('Aritzia brand not found in database. Run add-aritzia-brand.sql first.');
    }
    ...
    for (const item of products) {
      try {
        ...
        const productData = {
          brand_id: brand.id,
          external_id: String(item.mpn || item.additionalProperties?.sku || item.sku || item.productID || `aritzia-${Date.now()}-${Math.random()}`),
          name: item.name,
