-- Migration: Fix Empty Feed Issue
-- Date: December 30, 2025
-- Purpose: Handle NULL user_prefs gracefully and add debugging
...
BEGIN
  ...
  -- Get user preferences (may be NULL)
  SELECT * INTO user_prefs FROM user_preferences WHERE user_id = target_user_id;
  
  -- Safely check if user has any preferences
  IF user_prefs IS NOT NULL THEN
    has_preferences := COALESCE(user_prefs.total_likes, 0) > 0 OR COALESCE(user_prefs.total_follows, 0) > 0;
  END IF;
  
  -- Don't calculate offset here - we'll do it after brand limiting
  -- Just use offset_val directly with modulo based on refresh_seed for variety
  ...
  scored_products AS (
    SELECT 
      ...
      -- BRAND AFFINITY (30% weight) - handle NULL user_prefs
      CASE 
        WHEN has_preferences AND user_prefs IS NOT NULL THEN
          COALESCE((user_prefs.preferred_brands->>p.brand_id::text)::numeric, 0) * 3.0
        ELSE 0
      END as brand_score,
      
      -- CATEGORY MATCH (25% weight) - handle NULL user_prefs
      CASE 
        WHEN has_preferences AND user_prefs IS NOT NULL AND p.taxonomy_id IS NOT NULL THEN
      ...
      -- PRICE MATCH (5% weight) - handle NULL user_prefs
      CASE 
        WHEN has_preferences AND user_prefs IS NOT NULL AND user_prefs.avg_price IS NOT NULL THEN
          CASE 
      ...
      CASE 
        ...
        WHEN ri.days_since_shown < 1 THEN -80
        WHEN ri.days_since_shown < 2 THEN -60
        WHEN ri.days_since_shown < 3 THEN -45
        WHEN ri.days_since_shown < 7 THEN -30
        WHEN ri.days_since_shown < 14 THEN -20
        WHEN ri.days_since_shown < 21 THEN -10
        WHEN ri.days_since_shown < 30 THEN -5
        ELSE 0
...
DO $$
BEGIN
  RAISE NOTICE '=== Fix Empty Feed Migration Complete ===';
  RAISE NOTICE '';
  RAISE NOTICE 'Changes:';
  RAISE NOTICE '  - Added NULL safety for user_prefs';
  RAISE NOTICE '  - Added debugging logs (check Postgres logs)';
  RAISE NOTICE '  - Handles new users without preferences gracefully';
END $$;
