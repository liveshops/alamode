#!/usr/bin/env node
/**
 * Import Anthropologie products from Apify dataset
 * 
...
if (!datasetId) {
  ...
  console.log('Usage: node scripts/import-anthropologie-dataset.js <dataset_id>');
  process.exit(1);
}
...
async function importDataset() {
  console.log('🏷️  Importing Anthropologie products from Apify dataset...');
  console.log(`📦 Dataset ID: ${datasetId}\n`);

  try {
    // Get Anthropologie brand from database
    ...
    if (brandError || !brand) {
      throw new Error('Anthropologie brand not found in database. Add it with SQL first.');
    }
    ...
    for (const item of products) {
      try {
        ...
        const productData = {
          brand_id: brand.id,
          external_id: String(item.mpn || item.additionalProperties?.sku || item.sku || item.productID || `anthropologie-${Date.now()}-${Math.random()}`),
          name: item.name,
