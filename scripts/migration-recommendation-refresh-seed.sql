-- Migration: Add Refresh Seed for Fresh Feeds
-- Date: December 18, 2025
-- Purpose: Allow pull-to-refresh to show different products each time

-- Drop old function signature to avoid PGRST203 ambiguity error
DROP FUNCTION IF EXISTS get_recommendations(UUID, INTEGER, INTEGER);

CREATE OR REPLACE FUNCTION get_recommendations(
  target_user_id UUID,
  result_limit INTEGER DEFAULT 50,
  offset_val INTEGER DEFAULT 0,
  refresh_seed INTEGER DEFAULT 0  -- New parameter: changes ordering on refresh
)
RETURNS TABLE (
  product_id UUID,
  name TEXT,
...
BEGIN
  ...
  WITH scored_products AS (
    SELECT 
      ...
      -- Variety score: combines daily rotation + refresh seed for shuffling
      -- Uses product ID hash combined with seed to create consistent but different orderings
      (
        (EXTRACT(DOY FROM NOW())::int + refresh_seed + 
         (('x' || substr(md5(p.id::text), 1, 8))::bit(32)::int % 1000)
        ) % 10
      ) as variety_score
  ...
  ranked_products AS (
    SELECT 
      ...
      CASE 
        ...
        ELSE 'Popular pick'
      END as reason,
      -- Rank products within each brand by score (includes variety for shuffling)
  ...
  brand_limited AS (
    SELECT 
      rp.*,
      -- Interleave: show 1 from each brand before 2nd from any
...
-- Test: Different seeds should return different orderings
-- SELECT product_id, name FROM get_recommendations('<user-id>', 10, 0, 0);
-- SELECT product_id, name FROM get_recommendations('<user-id>', 10, 0, 1);
